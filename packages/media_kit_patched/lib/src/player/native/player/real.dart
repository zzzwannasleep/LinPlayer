...
class NativePlayer extends PlatformPlayer {
  ...
  Future<void> _create() {
    return lock.synchronized(() async {
      ...
      // Apply user-provided raw mpv options early (pre-initialize).
      final userOptionKeys = <String>{};
      for (final opt in configuration.extraMpvOptions) {
        if (opt.trim().isEmpty) continue;
        final idx = opt.indexOf('=');
        if (idx > 0) {
          final k = opt.substring(0, idx).trim();
          final v = opt.substring(idx + 1).trim();
          if (k.isNotEmpty) options[k] = v;
          if (k.isNotEmpty) userOptionKeys.add(k);
        } else {
          final k = opt.trim();
          options[k] = 'yes';
          userOptionKeys.add(k);
        }
      }
      ...
      final properties = <String, String>{
        if (!userOptionKeys.contains('idle')) 'idle': 'yes',
        if (!userOptionKeys.contains('pause')) 'pause': 'yes',
        if (!userOptionKeys.contains('keep-open')) 'keep-open': 'yes',
        if (!userOptionKeys.contains('audio-display')) 'audio-display': 'no',
        if (!userOptionKeys.contains('network-timeout')) 'network-timeout': '5',
        ...
        if (!userOptionKeys.contains('scale')) 'scale': 'bilinear',
        if (!userOptionKeys.contains('dscale')) 'dscale': 'bilinear',
        if (!userOptionKeys.contains('dither')) 'dither': 'no',
        if (!userOptionKeys.contains('cache')) 'cache': 'yes',
        if (!userOptionKeys.contains('cache-on-disk')) 'cache-on-disk': 'yes',
        if (!userOptionKeys.contains('hr-seek')) 'hr-seek': 'yes',
        if (!userOptionKeys.contains('hr-seek-framedrop')) 'hr-seek-framedrop': 'no',
        if (!userOptionKeys.contains('correct-downscaling')) 'correct-downscaling': 'no',
        if (!userOptionKeys.contains('linear-downscaling')) 'linear-downscaling': 'no',
        if (!userOptionKeys.contains('sigmoid-upscaling')) 'sigmoid-upscaling': 'no',
        if (!userOptionKeys.contains('hdr-compute-peak')) 'hdr-compute-peak': 'no',
        ...
        // OpenSL ES audio output seems to be broken on some of these.
          ...
          'ao': 'null',
        if (!userOptionKeys.contains('subs-fallback')) 'subs-fallback': 'yes',
        if (!userOptionKeys.contains('subs-with-matching-audio'))
      ...
      properties.addAll(
        {
          ...
          if (!userOptionKeys.contains('title')) 'title': configuration.title,
          if (!userOptionKeys.contains('demuxer-max-bytes'))
            'demuxer-max-bytes': configuration.bufferSize.toString(),
          if (!userOptionKeys.contains('demuxer-max-back-bytes'))
            'demuxer-max-back-bytes': configuration.bufferSize.toString(),
          if (!userOptionKeys.contains('vo') && configuration.vo != null) 'vo': '${configuration.vo}',
          if (!userOptionKeys.contains('demuxer-lavf-o'))
            'demuxer-lavf-o': [
            'seg_max_retry=5',
            'strict=experimental',
          ...
          if (!userOptionKeys.contains('sub-ass')) 'sub-ass': configuration.libass ? 'yes' : 'no',
          if (!userOptionKeys.contains('sub-visibility'))
            'sub-visibility': configuration.libass ? 'yes' : 'no',
          if (!userOptionKeys.contains('secondary-sub-visibility'))
