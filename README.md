...
- 片头跳过：服务端提供片头区间时，播放进入片头会提示是否跳过（一键跳到片头结束；可在设置开启）。
...
- 聚合服务：跨服务器继续观看 + 聚合搜索；聚合搜索在「按确认/搜索键」或「停止输入 1.5 秒」后触发，减少服务端压力。
...
- 「不限制视频流缓存」仅影响 MPV 在线播放：会持续把网络流写入磁盘缓存，并尽量缓存到结束（包含回退缓冲）；可能被服务器误判为下载且占用磁盘空间；可在同页「清理视频流缓存」删除。

### 片头跳过
设置入口：设置 → 播放 → 自动跳过片头

- 开启后，当服务端提供片头区间数据时（如 `IntroTimestamps` 或章节），播放进入片头会提示「跳过/不跳过」。
- 点击「跳过」将直接跳到片头结束时间点；若服务端不提供数据，则不会提示。
