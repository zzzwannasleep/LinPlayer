...
/// 标记已看/未看。
///
/// ★ 手动标「看过」也要同步 Bangumi:在这之前只有播到 80% 走 stop_playback 那条路才会同步,
...
async fn mark_bangumi_watched(acc: &linplayer_core::sync::SyncAccount, info: &emby::ScrobbleInfo) {
    ...
    // 只打「反查不到」等于什么都没说,用户问「为什么没点格子」时照样查不出来。
    let r = match matched {
        Ok(r) => r,
        Err(e) => {
            poclog(&format!("[Bangumi] 反查失败,跳过标记({}):{e}", info.title));
            return;
        }
    };
    ...
    let say = |what: &str, r: Result<(), String>| match r {
        Ok(()) => poclog(&format!("[Bangumi] {what} OK")),
        Err(e) => poclog(&format!("[Bangumi] {what} 失败: {e}")),
    };
    if info.media_type == "movie" {
        say(
            &format!("电影标记看过 subject={}", r.subject_id),
            bangumi::set_collection_type(acc, r.subject_id, 2).await,
        );
        return;
    }
    say(
...
async fn bangumi_set_collection(
    ...
    match bangumi::set_collection_type(&acc, subject_id, type_).await {
        Ok(()) => Ok(true),
        Err(e) => {
            poclog(&format!("[Bangumi] 设置收藏失败: {e}"));
            Ok(false)
        }
    }
}
...
async fn bangumi_update_episode(
    ...
    match bangumi::update_episode_status(&acc, subject_id, episode_id, type_.unwrap_or(2)).await {
        Ok(()) => Ok(true),
        Err(e) => {
            poclog(&format!("[Bangumi] 更新单集失败: {e}"));
            Ok(false)
        }
    }
}
