...
## 通用配置（免密码 · 跨客户端，Richasy 兼容，H14）

为了**任何兼容客户端都能免密码导入/导出**,提供与 Richasy/Rodel `common-config` 对齐的格式。
这是导出对话框里的**默认推荐项**。

> ⚠️ **混淆级加密**:解密密钥随文件分发(`_key`)或内置在客户端里,能挡住随手读取明文
> 凭据,但**不防被提取密钥后解密**。需要真正保密请用上文的口令模式或「加密给设备」。
...
- 每个服务器配置先序列化成 **snake_case JSON**(`type/id/name/url/username/user_id/password/`
  `access_token/icon/lines/options`),再用 **AES-256-CBC/PKCS7** 加密,**IV = 密钥前 16 字节**,
  base64 后放入 `configs[]`。
