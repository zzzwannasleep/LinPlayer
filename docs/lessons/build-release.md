...
## 更新说明的「对比基准」:两个渠道不是同一个(2026-09-13)

用户:「为什么更新日志一直对比 730 版本,应该是对应上一个版本的」。

`scripts/release-notes.sh` 原来一律取**上一个正式版** tag 当基准,而正式版很少发 ——
于是 build731~742 这十几个预发布**每一版都写「对比 v1.1.0-build730」**,清单一版比一版长
(742 那版列了 20 条,其实它只比 741 多 1 个提交)。用户分不出这一版到底改了什么。

两个渠道的用户上一个装到的版本不同,基准也就不同:

- **预发布(build.yml)用 `pre`**:`git describe --tags --abbrev=0 --match 'v*' HEAD~1`,
  上一个发布,预发布或正式版都算。
- **正式版提升(publish.yml)用 `stable`**:同上再加 `--exclude '*-pre'`。正式版渠道的人
  没装过中间那些预发布,只对比上一个预发布会漏掉整段。原来 publish.yml **照搬预发布的说明**,
  改成基准之后照搬就错了,所以它现在 checkout 预发布那个提交、按 stable 重新算。
- 从 **HEAD~1** 往回找:这一版自己的 tag(重跑时可能已经打上)不能算「上一个」。
- 用 `git describe`(按提交图找最近的祖先 tag),不用按 tag 名字排序:名字排序在
  `-pre` 后缀、构建号位数变化时都靠不住。

☠ 顺带钉住:正式版 tag 原来**没指定 target_commitish**,默认打在 main 最新 —— 提升期间
main 又进了提交的话,下一版算「对比上一个正式版」会把那些从没发布过的提交当成已经发过。
现在钉在预发布那个提交上。

实测(真 tag,worktree 里跑):742-pre 提交上 `pre` → 对比 741-pre、1 次提交;
`stable` → 对比 730、20 次;HEAD(751)上 `pre` → 对比 742-pre、9 次,`stable` → 对比 730、29 次。
publish.yml 那一半**没有真跑**:手动触发会真的发一个正式版,等下一次正式发布时再看它的说明。
