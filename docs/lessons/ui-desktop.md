...
### ☠☠ mpv 渲染完不还原 FBO:自己读像素前必须绑回去

做进度条缩略图要在 `OnOpenGlRender` 里读一帧回来。回调进来时 Avalonia 确实
把 `fb` 绑好了 —— 但中间 `lp_gl_render` 跑过一趟,**mpv 内部会绑自己的 FBO,
而且不保证还原**。于是 `glReadPixels` 读的是另一块缓冲区。

结果不是报错、不是崩,是**每一张缩略图都全黑**。尺寸对、对象非空、
张数也对得上时间轴 —— 除了内容,一切正常。

```
不绑:第 0/1/2 格 <- 1920x1200 红通道均值 0
绑了:第 0/1/2 格 <- 1920x1200 红通道均值 110
```

修法一行:读之前 `gl.BindFramebuffer(GL_FRAMEBUFFER, fb)`。

### 「有图」不等于「图是对的」

上面那个全黑版本,**通过了我一开始写的全部断言**:
「放过的位置有图」✓、「没放到的位置没有图」✓、尺寸 160×90 ✓。
我是看截图才发现气泡里那块是黑的。

所以判据补了一条**亮度均值**(抓帧时顺手隔 37 个像素采一次,几乎不要钱)。
注入「不绑 FBO」→ `✗ 读回来是全黑(亮度均值 0.0)`。

★ 一般化:**凡是「生成一张图 / 一段音 / 一个文件」的功能,断言不能停在
「产物存在且尺寸对」**。存在性是最容易凑出来的那一半,而内容错了通常不报错。

### 缩略图的帧只能来自「已经渲染过的」—— 这不是妥协

用户 2026-09-03:「做一个缩略图的功能,**缓存了的进度条能用,没缓存的不能用**」。

这条约束直接决定了实现,因为我们**没有**可以随便取任意时刻画面的东西:

| 路子 | 为什么不行 |
|---|---|
| 服务端 trickplay(BIF) | 只有新版 Emby 有,而本仓要伺候一堆 fork |
| 让 mpv 跳过去截一张 | 会把正在放的画面拽走 |
| 另起一个 mpv 实例解码 | 把播放器的复杂度翻一倍,去换一张小图 |

所以帧来自**我们自己已经渲染过的那些**:`OnOpenGlRender` 里顺手把当前帧
缩成 160×90 收进来。于是「有缩略图的位置」= 「已经放过的位置」——
这正好就是用户说的那条规则,而且它是**真的**,不是我们承诺出来的。

三个必须做对的细节:

- **按格存,不按秒**。两小时的片子按秒是 7200 张。切 300 格之后长片格粗、短片格细,
  而内存占用是个常数(300 × 160×90×4 ≈ 17 MB)。
- **行序要翻**:`glReadPixels` 原点在左下,位图第一行是画面最上面那一行。
  不翻的结果是每张都上下颠倒,**而它不报错**。
- **通道要换**:读回来 RGBA,Avalonia 的 `Bgra8888` 要 B,G,R,A。不换的话红蓝互调。

★ 还画了一条 2px 的带子标出「哪一段有图」(只在悬停时画)。
既然规则是「有的能用有的不能」,就不能让用户靠试 ——
划过去没图的时候,他分不清是「这儿没有」还是「这功能坏了」。

★ 判据要量**明确已经放过**的位置(播放头往回一半),不是播放头本身:
`_position` 是轮询来的,比真实渲染位置滞后一拍,而一格只有 duration/300 宽 ——
拿它去问「当前这一格有没有图」,问的其实是**正在填的那一格**,会随机红。
判据自己和被测对象抢时序,是假红和假绿的同一个来源。
