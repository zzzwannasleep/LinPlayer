...
OSD 淡入淡出都只能以 24fps 跑 —— 这是「播放页的按钮每个都会闪」的机制。

**默认仍然是 1。** block=0 那一列各项指标都更好(黑帧 ✓、avsync 0.0、丢帧 0、
出帧 24.2fps),唯一变差的是 `frame_jitter_ms`(2.7 → 5.9ms),而那是合成网格的量化
(94Hz 的网格上放 24fps,3 个心跳还是 4 个)。**但这条轴上我 2026-09-05 已经判错过一次**,
拿数说服自己改了默认值,用户实测打回。所以留成 `LP_BLOCK_FOR_TARGET_TIME=0`,
由眼睛判,不由我判。
