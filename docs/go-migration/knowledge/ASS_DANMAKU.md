...
> ⚠️ **2026-09-02 已从仓库删除**(用户决定:播放器不能有「弹幕要自己部署个服务才有」
> 这种选项,弹幕改回客户端内置)。本节留作**方法参考** —— 里面的签名口径、
> `merge_comments` 的并集语义、缓存 `now` 由调用方传入这几条,对客户端侧同样成立。
> 代价也要记住:**配额被刷完那条风险回来了**,而客户端限流拦不住外人(见 5.x)。
> 下面提到的所有 `crates/danmaku-proxy/*` 路径在当前工作区**都已不存在**,
> 要看代码请翻 git 历史。
