...
| G3 | 视频与 UI 在**同一个顶层窗口内**合成 | 不存在"两个顶层窗口手动对齐几何"这种东西。**⚠️ 这条不得以牺牲零拷贝硬解为代价达成**(§7.3 实测) |
...
> v0.1 写的是「**默认赌 B**」。**2026-08-31 实测推翻了这个默认值**,详见
> [`spikes/SPIKE-1b-zero-copy.md`](spikes/SPIKE-1b-zero-copy.md)。

**在 Windows 上,路径 B 保得住自绘 OSD,但保不住零拷贝硬解。**

**实测(Intel UHD 770 核显 —— 只有核显的机器是主场景,不是边角料):**

| 场景 | 路径 A(零拷贝) | 路径 B(`d3d11va-copy`) |
|---|---|---|
| `hwdec-current` | **`d3d11va`** | 最好只到 **`d3d11va-copy`** |
| 1080p60 → 1080p | 推进 1.001 ✅ · CPU **11%** | 推进 1.005 ✅ · CPU **48%** |
| 4K60 → 1080p | 推进 0.998 ✅ · CPU **7%** | 推进 **0.862 ❌** · CPU 50% · 渲染 **12.2 fps** |
| 4K60 → 4K | 推进 0.999 ✅ · CPU **7%** | 推进 **0.924 ❌** · CPU 57% · 渲染 **14.2 fps** |

同一个 libmpv、同一块 GPU、同一个片子、同样的输出分辨率。
**核显上路径 B 的 4K 是"播不了",不是"慢一点"**;1080p 跑得住但 CPU 是 4.4 倍。
语料还是 H.264 8-bit —— 真实 4K HDR 是 HEVC 10-bit,往返字节更多,只会更差。
**换成 ANGLE(EGL over D3D11)也拿不回来** —— 根因是这个 mpv 里
`d3d11egl` 与 `EGL_ANGLE_d3d_texture_client_buffer` **一个都没有**
(现代 mpv 移除了 ANGLE),而 render API **只有 OpenGL 与 SW 两种类型,没有 D3D11 类型**。

> 独显上 B 大概率跑得动 4K60,但**零拷贝与否是结构差异,换块好卡只是把它盖住**。
> 而且独显笔记本平时也跑在核显上(R10:不钉独显就是核显),**钉独显在 Linux 侧还没实现**。

#### 新的裁决顺序

```
1. 先做 D(DirectComposition)     <- 唯一可能三样全有:单窗口 + 自绘 OSD + 零拷贝
2. D 不通 -> 退 A2(双顶层窗口)   <- 已在产验证,代价是几何对齐,坑已知已记录
3. B 只在 Linux 上仍是候选         <- vaapi / nvdec 有成熟 GL 互操作,须单独实测
4. C 不进生产
```

**注意 A1(子窗口 `--wid`)从来不是候选** —— 它逼你用 mpv 自己的 Lua OSD,已被明确否决。
所以「OSD 好看」与「解码流畅」**从来不在同一条取舍轴上**;
真正的轴是「几何对齐的复杂度 vs 解码路径的退化」。
...
| R14 | **路径 B 在 Windows 上拿不到 d3d11va 零拷贝** | 实测**核显上 4K 播不了**(60fps 源只渲出 12~14 fps);1080p 跑得住但 CPU 是 4.4 倍 | 已确认结构性(ANGLE 也拿不回;mpv 无 `d3d11egl`、render API 无 D3D11 类型)。改走方案 D,不通则退 A2 | 🔴 已确认+已量化 |
