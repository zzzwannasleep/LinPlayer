...
func pumpStatus() {
	...
	for !drainStop.Load() {
		...
		q.push(&event{T: "event", Name: "player.status",
			mergeKey: "player.status",
...
func propF(name string) float64 {
	var f float64
	_, _ = fmtSscan(prop(name), &f)
	return f
}

// ---------------------------------------------------------------- 通道 B 的 5 个

func glInit(getProcAddress unsafe.Pointer, ctx unsafe.Pointer) C.int32_t {
	...
	if C.lp_rc_create(&out, mpvH, getProcAddress, ctx) < 0 {
		logf("error", "mpv_render_context_create 失败")
		return eInternal
	}
	rctx = out
	rctxSet.Store(true)
	logf("info", "视频通道 B 已就绪(render context 已建立)")
	return eOK
}

func glWantsRedraw() C.int32_t {
	mpvMu.Lock()
	rc := rctx
	mpvMu.Unlock()
	if rc == nil {
...
func glRender(fbo C.uint32_t, w, h, flipY C.int32_t) C.int32_t {
	mpvMu.Lock()
	rc := rctx
	mpvMu.Unlock()
	if rc == nil {
...
func playFile(path string) error {
	if r := ensureMpv(); r != 0 {
		return errStr("mpv 起不来")
	}
	if !waitRenderCtx(5 * time.Second) {
		return errStr("视频通道未就绪:UI 还没调 lp_gl_init。起播必须排在它之后(SPEC §7.2 约束 6)")
	}
	mpvMu.Lock()
	h := mpvH
	mpvMu.Unlock()
