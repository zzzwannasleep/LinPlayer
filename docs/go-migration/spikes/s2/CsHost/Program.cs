// SPIKE-2 · S2.2:C# 侧用 LibraryImport 把 Go 核心的 13 个导出调通。
//
// 这不是 demo,是判据集合。每条判据失败就计一次红,退出码 = 红的条数。
...
internal static class Program
{
    ...
    {
        var dll = args.Length > 0 ? args[0] : "../out/lpcore.dll";
        ...
        t6.Wait(3000);
        Check(t6.IsCompletedSuccessfully && t6.Result.GetProperty("err").GetProperty("code").GetString() == "E_NOTFOUND",
              "未知命令 -> E_NOTFOUND");
        ...
        // ---- 第 2、3 组导出可调 ----
        Console.WriteLine("== 8. 视频通道两组导出都调得进去 ==");
        Check(Core.lp_set_surface(0, 0, 0, 0) == 0, "lp_set_surface(解绑)可调");
        Check(Core.lp_gl_init((nint)0x1234, nint.Zero) == 0, "lp_gl_init 可调");
        Check(Core.lp_gl_wants_redraw() == 0, "lp_gl_wants_redraw 返回 0(桩:没有新帧)");
        Check(Core.lp_gl_render(1, 1920, 1080, 1) == 0, "lp_gl_render 可调");
        Core.lp_gl_swapped(); Core.lp_gl_uninit();
        Check(true, "lp_gl_swapped / lp_gl_uninit 可调");
        ...
        Console.WriteLine($"== 9. 内存所有权:2 万次 call/free{(leakMode ? "  【--leak 反向注入:故意不 free】" : "")} ==");
        const int N = 20000;
        for (var i = 0; i < N; i++)
        {
        ...
        Thread.Sleep(400);
        var diag = Diagnostics(pump);
        long alloc = diag.GetProperty("cstrAlloc").GetInt64();
        long freed = diag.GetProperty("cstrFree").GetInt64();
        ...
        else
        {
            ...
            Check(alloc >= N, "确实发生了 2 万次以上的跨边界分配", $"alloc={alloc}");
        }
