# -*- coding: utf-8 -*-
"""S1.2 附属实验:report_swap 的 A/B —— **裸 harness 版**(无 vsync 的忙轮询)。

派生自 `../spike-1b-glbackend.py`,唯一的改动是把
`mpv_render_context_report_swap()` 换成环境变量门控(LP_REPORT_SWAP=0 关掉)。
原件一字不动 —— 那是已结题 SPIKE 的记录。

存在的理由:Avalonia 侧实测「漏调 lp_gl_swapped 帧率不掉」,和 SPIKE-1b §7 第 3 条
相反。要判断是「那条结论错了」还是「宿主的呈现模型不同」,就得在**同一台机器、
同一个片子**上把 SPIKE-1b 的条件复现一遍。见 S1.2 报告 §5.4。

用法: LP_REPORT_SWAP=0|1 LP_BACKEND=wgl python swap-ab.py <clip> [宽 高]
"""
...
def run(backend, hwdec):
    ...
    while time.time()-t0<RUN_S:
        ...
        if m.mpv_render_context_update(rctx)&1:
            ...
            if REPORT_SWAP: m.mpv_render_context_report_swap(rctx)
            frames+=1
...
REPORT_SWAP = os.environ.get("LP_REPORT_SWAP","1") != "0"
log("report_swap = %s"%("开" if REPORT_SWAP else "★ 关(反向验证)"))
