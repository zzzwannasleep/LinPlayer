...
| 命令契约 | **168 / 271**(62%) | — | 103 | `bash scripts/check-bindings.sh` |
| 桌面页面 | **14 / 18** | — | 4 | `UI_PC.md` §7 逐页对 `apps/windows/.../Views/` |
...
| `sync.*` | **15 / 15** | — |
| `download.*` | **8 / 8** | — |
| `emby.*` | 38 / 40 | 观看记录扫描恢复(`watch_history_sync.rs` 719 行) |
| `player.*` | 35 / 39 | 外部播放器、播放窗开关、待播条目中转 |
| `prefs.*` | 19 / 25 | CF 测速、配置二维码进出、图标库、翻译设置 |
| `account.*` | 14 / 21 | 批量添加(`server_batch.rs` 694 行)、深链、账号图标 |
| `system.*` | 11 / 13 | 检查更新(`update.rs` 603 行)、赞助校验 |
| `plugin.*` | **0 / 22** | 整个插件引擎(要 JS 运行时) |
| `translate.*` | **0 / 9** | `translation.rs` 2827 行 |
| `anirss.*` | **0 / 51** | C24 已裁剪范围:只做播放,不做管理台 |

★ 剩余 103 条里 **82 条集中在三个域**(anirss 51 / plugin 22 / translate 9),
其余 21 条是散在已有命名空间里的小口。桌面 UI 还缺的 4 个页面
(影视目录 / 插件市场 / Ani-RSS / 图标库)全部卡在 `plugin.*` 和 `anirss.*` 上
—— **不是 UI 欠的**。
