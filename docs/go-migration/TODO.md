...
| 维度 | 完成 | 进行中 | 未开始 | 怎么验 |
|---|---|---|---|---|
| 条目总数 | **44** | 12 | 180 | 数本文件的 `- [x] **ID**` |
| 命令契约 | **187 / 271**(69%) | — | 84 | `bash scripts/check-bindings.sh` |
| 命令契约(**扣掉已裁范围**) | **187 / 220**(85%) | — | 33 | anirss 那 51 条按 C24b 不做 |
| 桌面页面 | **15 / 18** | — | 3 | `UI_PC.md` §7 逐页对 `apps/windows/.../Views/` |
| 桌面页面(**扣掉已裁范围**) | **15 / 17** | — | 2 | Ani-RSS 管理台按 C24b 不做 |
| 差分对账 | **18 条全绿** | — | — | `bash scripts/check-core.sh` 第 5 关 |

> ★ 命令契约这一格**不要手数**。`COMMANDS.md` 每行的「移植」勾是生成器读
> Go 注册表算出来的(`scripts/gen-commands.py`),`check-bindings.sh` 第 1 关
> 会校验它是最新的 —— 手打的勾会被下一次生成冲掉,而且会和事实分家。

**按端看,进度是极不平均的 —— 这是有意的:**

| 端 | 状态 |
|---|---|
| **Windows** | 🟢 出得了绿色包,SPEC 里除插件与字幕翻译外**全部落地** |
...
| `sync.*` | **15 / 15** | — |
| `download.*` | **8 / 8** | — |
| `prefs.*` | 23 / 25 | 只差翻译设置两条(跟着 `translate.*` 一起做) |
| `plugin.*` | **0 / 22** | 整个插件引擎(要 JS 运行时,quickjs-go 一类) |
| `translate.*` | **0 / 9** | `translation.rs` 2827 行 + Whisper |
| `anirss.*` | **0 / 51** | **按 C24b 不做**(范围已砍:只做播放,已作为媒体源落地) |

★ 剩下真正要做的只有两块:**插件引擎(22 条 + 插件市场页 + 影视目录页)**
和**字幕翻译(9 + 2 条)**。anirss 那 51 条是管理台,C24b 已决定不移植 ——
播放那半已经作为 `core/source/anirss` 落地了。

桌面 UI 还缺的 2 个页面(影视目录 / 插件市场)都卡在 `plugin.*` 上,**不是 UI 欠的**;
第 3 个(Ani-RSS 管理台)按 C24b **不做**。
