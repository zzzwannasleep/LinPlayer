...
| 命令契约 | **220 / 271**(81%) | 51 | `bash scripts/check-bindings.sh` |
| 命令契约(**扣掉已裁范围**) | **220 / 220 · 满格** | 0 | anirss 那 51 条按 C24b 不做 |
...
| 端 | 状态 |
|---|---|
| **Windows** | 🟢 出得了绿色包;**SPEC 里在范围内的命令与页面全部落地** |
...
- [x] **W26** 进度条缩略图重做:从**本地已缓存的字节**解 —— 2026-09-03
  ...
  ⚠️ **这一轮没做**:
  - 看完的片子再点播放会直接落在片尾 EOF(真 Emby 看完会清续播位置,我们没有)
  - `scripts/selfcheck-win.sh` 里 `dotnet build` 失败没能拦住后续步骤,
    结果是拿**上一次编的 exe** 跑自检
  - 转码流没有本地字节,缩略图不可用(设计如此,进度条带子会是空的)
